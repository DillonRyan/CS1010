				...
				if(loan == 0) {
					...
					if(list.length != 0) {	
						...
						if(harness != null) {
							...
							JOptionPane.showMessageDialog(null, tmp.clubMember + " returned " + tmp.make + " " + tmp.modelNumber); 
				...
				//Remove
					...
					if(list.length != 0) {
						...
						if(harness != null) {
							...
							Harness tmp = records.findHarness((String)makeL[index], (String)modNoL[index]);
							JOptionPane.showMessageDialog(null, tmp.make + " " + tmp.modelNumber + " has been removed."); 
